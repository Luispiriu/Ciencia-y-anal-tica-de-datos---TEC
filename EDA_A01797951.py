...
num_cols = df[['Age', 'Experience', 'Income', 'Family', 'CCAvg', 'Mortgage']]

for col in num_cols.columns:
    g = sns.displot(num_cols[col], kde=True, bins=30, color="skyblue")
    g.set(title=f"Distribución de {col}")
    plt.show
...
for col in num_cols:
    ...
    plt.show()
"""
Analiza la posición de la media respecto a la mediana.
¿Qué indica esta relación sobre el sesgo (asimetría) de la distribución?
"""
#Respuesta
...
for col in cat_cols:
    ...
    # Si la variable tiene más de 10 categorías → mostrar solo top 10
